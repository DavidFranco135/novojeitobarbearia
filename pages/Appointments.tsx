      ...
      <div className={`flex-1 rounded-[2rem] shadow-2xl overflow-hidden flex flex-col border ${theme === 'light' ? 'bg-white border-zinc-200' : 'cartao-vidro border-white/5'}`}>
        {viewMode === 'grid' ? (
                        ...
                        {app ? (
                          <div className={`h-full w-full rounded-2xl border flex flex-col justify-between transition-all group ${app.status === 'CONCLUIDO_PAGO' ? 'border-emerald-500/40 bg-emerald-500/10' : app.awaitingOnlinePayment ? 'border-blue-400/50 bg-blue-500/10' : 'border-[#C58A4A]/30 bg-[#C58A4A]/5'} ${isBarbeiroRestrito ? 'p-3' : compactView ? 'p-1.5 rounded-lg' : 'p-2'}`}>
                            ...
                            <div className={`flex items-center justify-end gap-1 ${compactView ? 'mt-0.5' : 'mt-1'}`}>
                               ...
                               <button onClick={(e) => { e.stopPropagation(); const cl = clients.find((c: any) => c.phone === app.clientPhone || c.name === app.clientName); openClientProfile(cl || { id: null, name: app.clientName, phone: app.clientPhone, notes: '', photos: [] }); }} className={`bg-[#C58A4A]/20 text-[#C58A4A] hover:bg-[#C58A4A]/40 rounded-lg transition-all ${compactView ? 'p-0.5' : 'p-1'}`} title="Fotos e obs. do cliente"><Camera size={compactView ? 9 : 11}/></button>
          ...
          <div className="p-6 space-y-3 overflow-y-auto h-full scrollbar-hide">
             ...
             {appointmentsFiltered.filter((app: any) => !isBarbeiroRestrito || app.professionalId === myProfessionalId).map(app => (
               <div key={app.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-[#C58A4A]/30 transition-all">
                  ...
                  <div className="flex items-center gap-2">
                     ...
                     <button onClick={() => { const cl = clients.find((c: any) => c.phone === app.clientPhone || c.name === app.clientName); openClientProfile(cl || { id: null, name: app.clientName, phone: app.clientPhone, notes: '', photos: [] }); }} className="p-2 bg-[#C58A4A]/20 border border-[#C58A4A]/30 text-[#C58A4A] hover:bg-[#C58A4A]/40 rounded-xl transition-all" title="Fotos e observações do cliente"><Camera size={16}/></button>
